...
(async () => {
 ...
 const makeElement = () => ({ value: '', textContent: '', hidden: false, disabled: false, files: [], children: [], handlers: {}, attrs: {}, addEventListener(name, fn) { this.handlers[name] = fn; }, setAttribute(k,v) { this.attrs[k]=v; }, removeAttribute(k) { delete this.attrs[k]; }, replaceChildren(...children) { this.children = children; }, querySelectorAll(selector) { return selector === 'li' ? this.children : []; }, focus() {} });
 ...
 elements['pdf-upload'].files = [{ name: 'notes.txt', type: 'text/plain', size: 100 }]; await elements['pdf-upload'].handlers.change(); assert.match(elements.error.textContent, /Choose a PDF/);
 elements['pdf-upload'].files = [{ name: 'large.pdf', type: 'application/pdf', size: 6 * 1024 * 1024 }]; await elements['pdf-upload'].handlers.change(); assert.match(elements.error.textContent, /5 MB/);
 ...
 console.log('Client checks PASS: PDF validation, empty, oversized, loading, structured AI result, copy, stale-result clearing, server failure, unreadable response, theme, clear.');
